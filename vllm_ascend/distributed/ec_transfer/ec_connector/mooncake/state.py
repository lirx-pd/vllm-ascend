...
class SchedulerTransfer:
    ...
    Attributes:
        ...
        evicted_at: Eviction scheduled after dispatch but before the load ACK.
    """
...
class SchedulerTransferTable:
    ...
    def complete_load(self, mm_hash: str) -> bool:
        ...
        if record.evicted_at is not None:
            ...
            record.evicted_at = None
            self._release_ready_records(mm_hash, evicted_at)
        return True
    ...
    def release_ready(self, mm_hash: str, now: float) -> None:
        for record in self.records_for_hash(mm_hash, (SchedulerTransferState.LOADING,)):
            # A later batch can evict a dispatched load before its ACK arrives.
            # Loads queued in this batch run after its frees and remain valid.
            if record.transfer_id not in self._loads_to_dispatch:
                record.evicted_at = now
        self._release_ready_records(mm_hash, now)

    def _release_ready_records(self, mm_hash: str, now: float) -> None:
        ready = [
            record
            for record in self._records.values()
            if record.mm_hash == mm_hash and record.state is SchedulerTransferState.READY
        ]
        if ready:
            canonical = ready[-1]
    ...
    def expire(self, now: float, terminal_limit: int) -> tuple[list[SchedulerTransfer], int]:
        if terminal_limit < 0:
        ...
        for record in list(self._records.values()):
            ...
            if record.state is SchedulerTransferState.AVAILABLE:
                ...
                dropped += 1
        terminal_ids = [record.transfer_id for record in self._records.values() if record.state in _TERMINAL_STATES]
    ...
    def _check_identity(record: SchedulerTransfer, mm_hash: str) -> None:
        if record.mm_hash and record.mm_hash != mm_hash:
            raise ValueError(f"Transfer {record.transfer_id!r} changed mm_hash from {record.mm_hash!r} to {mm_hash!r}")
    ...
    def _transition(
        ...
        if state not in _ALLOWED_TRANSITIONS[record.state]:
            raise InvalidSchedulerTransferTransition(
                f"Cannot transition {record.transfer_id!r} from {record.state.name} to {state.name}"
            )
    ...
    def _evict_residents(self, now: float) -> None:
        while self.resident_bytes > self._resident_capacity:
            record = next(
                record for record in self._records.values() if record.state is SchedulerTransferState.RESIDENT
            )
            self._transition(record, SchedulerTransferState.EXPIRED, now=now)
