...
class MooncakeECConfig:
    ...
    @classmethod
    def from_vllm_config(cls, vllm_config: VllmConfig, role: ECConnectorRole) -> MooncakeECConfig:
        ...
        if parallel_config.pipeline_parallel_size != 1:
            raise ValueError("ECMooncakeConnector requires pipeline_parallel_size=1.")
        if parallel_config.data_parallel_size != 1:
            raise ValueError("ECMooncakeConnector requires data_parallel_size=1.")

        registered_buffer_size = _positive_integer("ec_buffer_size", ec_config.ec_buffer_size)

        extra = ec_config.ec_connector_extra_config
        raw_port = extra.get("reservation_zmq_port")
        reservation_port = _integer("reservation_zmq_port", raw_port) if raw_port is not None else None
        if reservation_port is not None and not 1 <= reservation_port <= 65535:
            raise ValueError("ECMooncakeConnector requires reservation_zmq_port in 1..65535.")

        if reservation_port is not None:
            reservation_port += parallel_config.data_parallel_index * parallel_config.tensor_parallel_size
            highest_port = reservation_port + parallel_config.tensor_parallel_size - 1
            if not 1 <= reservation_port <= highest_port <= 65535:
                raise ValueError("ECMooncakeConnector reservation ports must be in 1..65535.")
        ...
        if is_consumer and role == ECConnectorRole.SCHEDULER and not reservation_addr:
            raise ValueError(
                "ec_consumer with ECMooncakeConnector requires reservation_zmq_port or reservation_zmq_addr."
            )
        ...
        protocol = _nonempty_string("mooncake_protocol", extra.get("mooncake_protocol", "ascend"))
        if protocol != "ascend":
            raise ValueError("ECMooncakeConnector requires mooncake_protocol='ascend'.")
        ...
        buffer_device = (raw_buffer_device or "npu").strip() or "npu"
        # Upstream's fixed-version default is CUDA. The Ascend connector owns
        # this hardware boundary and maps that default to the current NPU.
        if buffer_device == "cuda":
            buffer_device = "npu"
        if not buffer_device.startswith("npu"):
            raise ValueError("ECMooncakeConnector requires an NPU buffer device.")
