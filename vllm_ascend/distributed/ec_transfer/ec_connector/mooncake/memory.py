...
resident reuse, NPU-safe retirement, and pressure-driven reclamation.
"""
...
def _allocate_registered_pool(
    capacity: int,
    device: torch.device,
) -> torch.Tensor:
    registered_size = math.ceil(capacity / _REGISTRATION_ALIGNMENT) * _REGISTRATION_ALIGNMENT
    storage = torch.empty(
    ...
    pool = storage.narrow(0, offset, registered_size)
    if pool.data_ptr() % _REGISTRATION_ALIGNMENT or pool.nbytes % _REGISTRATION_ALIGNMENT:
        raise RuntimeError("Mooncake NPU pool must be 2 MiB aligned")
    return pool
...
class ProducerMemoryPool:
    ...
    def _ensure_pool(self, device: torch.device) -> None:
        ...
        with self._lock:
            if self._pool is not None:
                return
            if device.type != "npu":
                raise ValueError("Mooncake EC producer pool requires an NPU device.")
            ...
            self._pool = pool
            self._allocator = ContiguousAllocator(self._capacity)
            logger.info(
    ...
    def stage(self, tensors: list[torch.Tensor]) -> StagedSources | None:
        """Copy tensors into one registered NPU slab."""
        if not tensors:
        ...
        with self._lock:
            for tensor in tensors:
                ...
                staged.append(pool.narrow(0, region[0], tensor.nbytes).view(tensor.dtype).view(tensor.shape))
...
class ConsumerMemoryPool:
    ...
    Attributes:
        ...
        _retire_events: NPU events guarding retired resident entries.
        _pending_frees: Allocations waiting for NPU consumers to finish.
    ...
    def __init__(
        ...
        self._retire_events: dict[str, torch.npu.Event] = {}
        self._pending_frees: list[tuple[torch.npu.Event, MemoryAllocation]] = []
    ...
    def prepare(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError("Could not initialize the EC consumer NPU buffer pool") from error
        self._pool = pool
        self._allocator = ContiguousAllocator(self._capacity)
        logger.info(
            "Prepared %d-byte NPU receive pool for Mooncake EC (registered=%s)",
            pool.nbytes,
            receiving_rank,
        )
    ...
    def _defer_or_free(self, allocation: MemoryAllocation, event: torch.npu.Event | None) -> None:
