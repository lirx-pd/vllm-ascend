...
class ProducerSourceLease:
    ...
    Attributes:
        ...
        ready_event: NPU event proving that production of the tensor finished.
    """
...
class ProducerPushRecord:
    ...
    Attributes:
        ...
        reservation_future: Future resolving the Consumer reservation.
        reservations: Resolved destination descriptors.
        io_futures: Data-plane futures that may still read the source.
    ...
    io_futures: list[Future[Any]] = field(default_factory=list)
...
class ProducerPushManager:
        ...
        with self._lock:
            ...
            if existing is not None:
                if existing.spec != spec:
                    raise ValueError(f"Producer transfer {spec.transfer_id!r} changed identity")
    ...
    def track_io_futures(
        ...
        with self._lock:
            for record in records:
                record.io_futures.extend(futures)
    ...
    def _check_sources_releasable(records: list[ProducerPushRecord]) -> None:
        for record in records:
            futures = [record.reservation_future, *record.io_futures]
            if record.source is not None and not all(future.done() for future in futures):
