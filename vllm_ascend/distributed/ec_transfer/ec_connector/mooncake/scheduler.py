...
from vllm_ascend.distributed.ec_transfer.ec_connector.mooncake.state import (
...
_CANCEL_ATTEMPTS = 2
_QWEN_VL_PLACEHOLDER_METADATA_FIELDS = {
...
class ECMooncakeScheduler:
    ...
    def from_vllm_config(cls, vllm_config: VllmConfig) -> ECMooncakeScheduler:
        ensure_mooncake_available()
        config = MooncakeECConfig.from_vllm_config(vllm_config, ECConnectorRole.SCHEDULER)

        control_client = ControlClient(config.control_timeout_ms)
        event_inbox = EventInbox(control_client)
        control_executor = ThreadPoolExecutor(
            max_workers=config.control_workers,
            thread_name_prefix="ec-mooncake-control",
        )
        encoder_cache_hidden_dim = _get_encoder_cache_hidden_dim(vllm_config) if config.is_producer else None
        return cls(
            config,
    ...
    def __init__(
        ...
        self._transfers = SchedulerTransferTable(self._consumer_pool_capacity, _LEASE_TTL_SECONDS)
    ...
    def _note_awaiting_push(
        ...
        self._transfers.mark_unavailable(transfer_id, "push readiness event timed out", now)
    ...
    def _expire_transfers(self) -> None:
        now = time.monotonic()
        expired, _ = self._transfers.expire(now, _MAX_TERMINAL_TRANSFER_RECORDS)
    ...
    def has_cache_item(self, identifier: str) -> bool:
        ...
        if self._transfers.has_state(identifier, (SchedulerTransferState.RESIDENT,)):
            return True
        return self._transfers.has_state(identifier, (SchedulerTransferState.AVAILABLE,))
    ...
    def ensure_cache_available(
        ...
        for index, feature in enumerate(request.mm_features):
            ...
            if transfer_id is not None:
                self._transfers.touch_available(transfer_id, time.monotonic() + _LEASE_TTL_SECONDS)
    ...
    def update_connector_output(self, connector_output: ECConnectorOutput) -> None:
        meta = getattr(connector_output, "ec_connector_worker_meta", None)
        ...
        for mm_hash in meta.failed_loads:
            self._transfers.fail_load(mm_hash, "worker failed to load", time.monotonic())
